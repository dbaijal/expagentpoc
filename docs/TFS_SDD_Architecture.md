...
AEM Assets (as a Cloud Service) provides the centralized **Digital Asset Management (DAM)** — the **authoring source** for images and rich media. Authors reference assets from the DAM while authoring; on publish, assets are **ingested into the EDS media bus and served from there at the edge**, where EDS applies image optimization (format, compression, responsive sizing). Assets are therefore delivered from the EDS media bus, not fetched live from the DAM at request time, and no Dynamic Media dependency is required. Where dynamic or shared elements are required at delivery, an **Edge Worker** operates at the edge as a lightweight middleware layer — for example injecting the shared **header/footer** (from the TFS header/footer microservice) — and **App Builder** (Adobe I/O Runtime) provides off-platform services/integrations that are invoked on demand. **Personalization (Adobe Target)** and **analytics (Web SDK / AEP)** are enabled through **client-side** integrations that run in the browser on delivered pages, keeping them decoupled from the authoring layer.
...
| **AEM Assets (DAM)** | Digital Asset Management — asset **authoring source** | Centralized asset repository; brand governance & reuse | Assets referenced by AEM content while authoring; on publish, ingested into the **EDS media bus** and served/optimized at the edge (no Dynamic Media dependency) |
...
| **TFS Header/Footer Microservice** | Shared global navigation & footer | Global header/footer generation for all locales | Header/footer injected into pages at the edge via the **Edge Worker** |
| **Adobe Target** | Personalization & experimentation | Audience-based personalization; A/B & multivariate testing; experience targeting | Runs **client-side** on delivered pages; decoupled from the authoring layer |
| **Analytics (Web SDK / AEP)** | Analytics & measurement | User-behaviour tracking; reporting; audience data (Web SDK, migrating from Adobe Analytics; AEP) | Runs **client-side** via JavaScript SDKs on delivered pages |
